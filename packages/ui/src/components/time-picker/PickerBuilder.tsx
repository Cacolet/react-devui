...
import { deepCompareDate } from '../time-picker/utils';
  ...
  return (
    ...
    >
      {({ tiStyle, tiOnMouseDown, tiOnMouseUp, ...restSProps }) => (
        <div {...restSProps} ref={popupRef} className={dPopupClassName} style={tiStyle} onMouseDown={tiOnMouseDown} onMouseUp={tiOnMouseUp}>
          {children({
            pbDate: isFocus[0] ? valueLeft : valueRight,
            pbCurrentDate: dRange ? (isNull(_value) ? dataRef.current.rangeDate : (_value as [Date, Date])) : [valueLeft, null],
            pbPosition: isFocus[0] ? 'start' : 'end',
            changeValue: (date) => {
              changeValue(date, isFocus[0] ? 'start' : 'end');
            },
          })}
        </div>
      )}
